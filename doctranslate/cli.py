...
def main():
    parser = argparse.ArgumentParser(description="DocTranslate - Local Document Conversion & Translation tool.")
    parser.add_argument("files", nargs="+", help="Paths to files to convert and translate (.pdf, .docx, .pptx, .epub, .html).")
    parser.add_argument("-li", "--lang-in", default="en", help="Source language code (default: en).")
    parser.add_argument("-lo", "--lang-out", default="vi", help="Target language code (default: vi).")
    parser.add_argument("-o", "--output-dir", help="Directory to save output files. Defaults to same directory as input.")
    parser.add_argument("--mode", choices=["mono", "dual", "both"], default="both", help="Output layout: mono, dual, or both (default: both).")
    parser.add_argument("--ollama-model", default="qwen2.5:8b", help="Local Ollama model name (default: qwen2.5:8b).")
    parser.add_argument("--ollama-url", default="http://localhost:11434/v1", help="Ollama API base URL (default: http://localhost:11434/v1).")
    parser.add_argument("--mineru-api", help="Optional URL to a local MinerU API server (e.g. http://localhost:8000/parse).")
    ...
    for file_path in args.files:
        ...
        if ext == ".pdf":
            # PDFs are handled asynchronously via BabelDOC
            asyncio.run(translate_pdf_file(file_path, args))
        elif ext in [".docx", ".pptx", ".epub", ".html", ".htm"]:
            # Other formats are handled via the Unified Document Model
